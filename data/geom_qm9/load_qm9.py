...
from data.config import GEOM_QM9_RDKIT_SUMMARY_PATH, RDKIT_FOLDER_DIR, \
    GEOM_QM9_CSV2JSON_PATH, GEOM_QM9_CSV_PATH, GEOM_QM9_PICKLE_PATH
...
def cache_qm9():
    print('Loading QM9...')
    df = pd.read_csv(GEOM_QM9_CSV_PATH)
    csv: np.ndarray = df.values

    o_keys = csv[:, 1]
    o_values = csv[:, 5: 17]

    fp = open(GEOM_QM9_CSV2JSON_PATH)
    csv2json: dict = json.load(fp)
    fp.close()
    fp = open(GEOM_QM9_RDKIT_SUMMARY_PATH)
    ...
    print('\tCaching QM9...')
    fp = open(GEOM_QM9_PICKLE_PATH, 'wb+')
...
def load_qm9(max_num: int = -1) -> Tuple[List[List[Tuple[float, Molecule]]], np.ndarray]:
    print('\tLoading QM9...')
    fp = open(GEOM_QM9_PICKLE_PATH, 'rb')
